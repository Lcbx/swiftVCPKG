...
void main(){
    ...
    float shadowDepth = texture(shadowMap, fragShadowTexCoord).r; // get the depth from the shadowmap at the transformed shadow position
    if(fragShadowDepth + 0.001 > shadowDepth){ // test fragShadowDepth against shadow sample
        ...
        finalColor = fragColor * lightColor * length(fragColor);
    }
}
